...
chrome.webNavigation.onCommitted.addListener(function(details) {
	if (details.frameId != 0) return;
	console.log("onCommitted: " + details.tabId + " | " + details.transitionQualifiers.join(',') + " | " + details.transitionType  +" | " + details.url)
});

chrome.webNavigation.onHistoryStateUpdated.addListener(function(details) {
	if (details.frameId != 0) return;
	console.log("onHistoryStateUpdated: " + details.tabId + " | " + details.transitionQualifiers.join(',') + " | " + details.transitionType  +" | " + details.url)
});


chrome.webRequest.onCompleted.addListener(function(details) {
	console.log("onCompleted: " + details.tabId + " | " + details.type  +" | " + details.url)
}, {urls: ["<all_urls>"], types: ["main_frame"]}); 

chrome.webRequest.onErrorOccurred.addListener(function(details) {
	console.log("onErrorOccurred: " + details.tabId + " | " + details.type + " | " + details.error + " | " + details.url)
}, {urls: ["<all_urls>"], types: ["main_frame"]}); 
 */
